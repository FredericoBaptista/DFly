...
def get_proof(serial_number, nonce, tree_name): 
    ...
    result = subprocess.run(['./circom_proof_test.sh', './circuits'],
    ...
    # Initialize variables to store time measurements
    witness_generation_time_ns = 0
    proof_generation_time_ns = 0
    json_output_str = ""
    parsed_output = []
    ...
    for line in script_output.splitlines():
        if "generation time was" in line:
            ...
            match = re.search(r'(\w+ generation time was (\d+) nanoseconds)', line)
            if match:
                if 'Witness' in match.group(1):
                    witness_generation_time_ns = int(match.group(2))
                elif 'Proof' in match.group(1):
                    proof_generation_time_ns = int(match.group(2))
        else:
            # This line is part of the JSON output
            json_output_str += line

    try: 
        # Split the output by '],[' and replace with '], ['
        # This is to ensure each sub-array is correctly formatted as JSON
        formatted_output = json_output_str.replace('],[', '], [')
        ...
        return witness_generation_time_ns, proof_generation_time_ns, parsed_output
...
def main():
    ...
    while i <= 180:
        ...
        excel_path = 'tests/TestMT.xlsx'

        # Load the existing Excel file
        df = pd.read_excel(excel_path, sheet_name='Tests')

        row = df[df['Test Number'] == 'Test ' + str(i)]

        depth = row['Depth Tree'].iloc[0]        
        
        
        if z != depth:
            
            initialize_tree(depth, "Test")
            insert_leaf("Test13876s", 110, "Test")
            z = depth



        witness_time, proof_time, proof = get_proof("Test13876s", 110, "Test")

        df.at[row.index[0], "Proof Time"] = proof_time
        df.at[row.index[0], "Witness Time"] = witness_time
